...
step1_2Btn.addEventListener("click", function () {
    ...
    errorArr.push(validateInput(nameInput, namePattern, "Enter valid full name."));
    errorArr.push(validateInput(emailInput, emailPattern, "Enter valid email address."));
    errorArr.push(validateInput(phoneInput, phonePattern, "Enter valid phone number."));
...
let invoiceTotalPlanTypeElem = document.getElementById("invoice_total_plan_type");
...
function updatePricesByType() {
    ...
    orderSummary.planPrice = subscriptionInfo.plansPrices[orderSummary.planName] || 0;
...
function populateInvoice() {
    ...
    if (orderSummary.addOns.length == 0) {
        ...
        orderSummary.addOns.forEach(element => {
            ...
            <span class="invoice-add-on-price">${priceFormat(element.price, null, "+$")}</span>
            `
